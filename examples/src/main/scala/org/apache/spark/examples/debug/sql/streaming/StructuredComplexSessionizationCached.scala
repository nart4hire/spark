        ...
        case (
          ...
          def handleEvict(sessions: List[SessionAcc]): Iterator[Session] = {
            ...
            val (evicted, kept) = sessions.span { s =>
              s.endTime.getTime < state.getCurrentWatermarkMs()
            }
          ...
          if (state.hasTimedOut && state.exists) {
            ...
            val sessionsFromEvents = events.map {
              case (userId, eventType, timestamp) =>
...
case class SessionAcc(events: List[SessionEvent]) {
  ...
  require(
    !sortedEvents.dropRight(1).exists(_.eventType == EventTypes.CLOSE_SESSION),
