 ...
 * Run with
 * {{{
 * bin/run-example debug.ml.LinearRegressionExampleNotCached [options]
 * }}}
 * A synthetic dataset can be found at `data/mllib/sample_linear_regression_data.txt` which can be
 * trained by
 * {{{
 * bin/run-example debug.ml.LinearRegressionExampleNotCached --regParam 0.15 --elasticNetParam 1.0 \
 *   data/mllib/sample_linear_regression_data.txt
 * }}}
 * If you use it as a template to create your own app, please use `spark-submit` to submit your app.
 */
object LinearRegressionExampleNotCached {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = new OptionParser[Params]("LinearRegressionExampleNotCached") {
      head("LinearRegressionExampleNotCached: an example Linear Regression with Elastic-Net app.")
      opt[Double]("regParam")
  ...
  private[ml] def loadDatasets(
    ...
    val training = dataframes(0)
    val test = dataframes(1)
  ...
  def run(params: Params): Unit = {
    val spark = SparkSession
      .builder
      .appName(s"LinearRegressionExampleNotCached with $params")
      .getOrCreate()

    println(s"LinearRegressionExampleNotCached with parameters:\n$params")
