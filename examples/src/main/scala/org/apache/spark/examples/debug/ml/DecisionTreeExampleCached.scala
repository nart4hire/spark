 ...
 */
object DecisionTreeExampleCached {
  ...
  private[ml] def loadDatasets(
    ...
    // Load training data
    val origExamples: DataFrame = loadData(spark, input, dataFormat).cache()
  ...
  def run(params: Params): Unit = {
    val spark = SparkSession.builder
      .appName(s"DecisionTreeExampleCached with $params")
      .getOrCreate()
    ...
    val dt = algo match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Algo ${params.algo} not supported."
        )
    }
    stages += dt
  ...
  private[ml] def evaluateRegressionModel(
    ...
    val fullPredictions = model.transform(data).cache()
    val predictions =
      fullPredictions.select("prediction").rdd.map(_.getDouble(0)).cache()
    val labels =
      fullPredictions.select(labelColName).rdd.map(_.getDouble(0)).cache()
