...
package org.apache.spark.examples.debug.streaming.clickstream
...
/** Analyses a streaming dataset of web page views. This class demonstrates
  * several types of operators available in Spark streaming.
  *
  * This should be used in tandem with PageViewStream.scala. Example: To run the
  * generator `$ bin/run-example
...
object PageViewStreamCached {
  def main(args: Array[String]): Unit = {
    ...
    val pageCounts = pageViews.map(view => view.url).countByValue().cache()
    ...
    val statusesPerZipCode = pageViews
      ...
      .groupByKey()
      .cache()
    val errorRatePerZipCode = statusesPerZipCode
      .map { case (zip, statuses) =>
