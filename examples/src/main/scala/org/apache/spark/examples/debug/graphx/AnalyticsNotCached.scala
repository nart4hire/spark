...
package org.apache.spark.examples.debug.graphx
    ...
    taskType match {
      case "pagerank" =>
        ...
        options.foreach { case (opt, _) =>
          throw new IllegalArgumentException(s"Invalid option: $opt")
        }
      ...
      case "cc" =>
        ...
        val sc = new SparkContext(
          conf.setAppName(s"ConnectedComponents($fname)")
        )
        val unpartitionedGraph = GraphLoader.edgeListFile(
          sc,
          fname,
        ...
        println(
          s"Components: ${cc.vertices.map { case (vid, data) => data }.distinct()}"
        )
        sc.stop()
