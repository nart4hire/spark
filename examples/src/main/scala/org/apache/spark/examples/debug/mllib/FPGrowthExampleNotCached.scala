 ...
 */
object FPGrowthExampleNotCached {
  ...
  def run(params: Params): Unit = {
    val conf = new SparkConf().setAppName(s"FPGrowthExampleNotCached with $params")
    val sc = new SparkContext(conf)
    val transactions = sc.textFile(params.input).map(_.split(" "))
