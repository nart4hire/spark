...
object SparkSQLExampleNotCached {
  ...
  private def runBasicDataFrameExample(spark: SparkSession, args: String): Unit = {
    ...
    val df = spark.read.json(args) // "examples/src/main/resources/people.json"
  ...
  private def runProgrammaticSchemaExample(spark: SparkSession, args: String): Unit = {
    ...
    // $example on:programmatic_schema$
    // Create an RDD
    val peopleRDD = spark.sparkContext.textFile(args) // "examples/src/main/resources/people.txt"
